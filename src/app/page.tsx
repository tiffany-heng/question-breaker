...
export default function QuestionBreaker() {
  ...
  const [data, setData] = useState<QuestionData>({ questionImageUrl: null, questionText: '', solutionImageUrl: null, solutionText: '', extractedText: '', variations: [] });
  ...
  useEffect(() => {
    const bootstrap = async () => {
      ...
      if (savedRoomId) {
        setRoomId(savedRoomId); setPairingCode(savedCode || '');
  ...
  const handleProcessWithAI = async () => {
    ...
    try {
      ...
      await supabase.from('questions').update({ extracted_text: result.extractedText, variations: result.variations, status: 'ready' }).eq('id', data.id);
  ...
  const handleSkipCrop = async () => {
    ...
    setStatus('uploading');
    try {
      // Force high-res JPEG conversion even on skip
      const img = new Image();
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col text-slate-900 font-sans pb-10">
      ...
      {!roomId ? (
        ...
        <div className="flex-1 flex flex-col relative">
          ...
          <main className="flex-1 flex flex-col md:flex-row overflow-hidden text-left">
            <div className="w-full md:w-1/2 border-r bg-slate-50/50 flex flex-col relative overflow-y-auto pb-40">
              ...
              <div className="absolute bottom-0 left-0 right-0 p-8 bg-gradient-to-t from-slate-50 via-slate-50 to-transparent flex flex-col items-center">
                ...
                {!(data.questionImageUrl || data.questionText) && <div className="flex items-center gap-3 text-slate-400 animate-pulse font-bold text-[10px] uppercase tracking-[0.2em] text-center"><Smartphone size={16} /> Awaiting Input from<br/>Host or Phone</div>}
              </div>
