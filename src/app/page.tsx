...
export default function QuestionBreaker() {
  ...
  const handleConfirmCrop = async () => {
    ...
    try {
      ...
      // THE FIX: Use natural dimensions for full quality
      ...
      }, 'image/jpeg', 1.0); // Use 1.0 for maximum quality
            ...
            {status === 'cropping' && imgSrc && (
              <div className="fixed inset-0 z-50 bg-slate-900 flex flex-col">
                <div className="p-4 flex justify-between items-center text-white border-b border-white/10 bg-slate-950">
                  ...
                  <span className="font-bold text-xs uppercase tracking-tighter text-indigo-400">Pinch or drag handles to crop</span>
                  <button onClick={handleConfirmCrop} className="bg-indigo-600 px-6 py-2 rounded-full font-black text-xs uppercase shadow-lg active:scale-95 transition-transform">Confirm</button>
                </div>
                ...
                <div className="p-6 bg-slate-950">
                   <button onClick={() => uploadToSupabase(rawFile!)} className="w-full p-4 bg-white/5 text-slate-400 text-[10px] font-black uppercase tracking-widest rounded-xl border border-white/10">Skip & Upload Original</button>
                </div>
              </div>
            )}

            {status === 'processing' || status === 'uploading' ? (
              <div className="space-y-4"><Loader2 className="w-12 h-12 text-indigo-600 animate-spin mx-auto" /><p className="font-bold text-lg">{status === 'uploading' ? 'Uploading High-Res...' : 'AI is Thinking...'}</p></div>
            ) : status === 'ready' ? (
            ...
            <div className="w-1/2 border-r bg-slate-50/50 flex flex-col relative">
              {data.imageUrl ? (<div className="flex-1 p-8"><img src={data.imageUrl} alt="Source" className="w-full h-full object-contain rounded-2xl shadow-2xl" /></div>) : (
                <div className="flex-1 flex items-center justify-center p-12 text-center max-w-xs mx-auto space-y-6">
                  ...
                  <div><h4 className="font-bold text-lg text-indigo-950">Connect Phone</h4><p className="text-slate-400 text-sm text-center">Join <span className="underline font-bold text-indigo-600">{sessionId}</span> to upload.</p></div>
                </div>
              )}
              {status === 'processing' && (
